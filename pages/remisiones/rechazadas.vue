...
const runStepByStep = async () => {
  ...
  // Get and run the driver for rejected referrals
  if (remisiones.value.length === 0){
    toast.add({ severity: "warn", summary: "No hay remisiones rechazadas", detail: "No se puede iniciar el asistente paso a paso, asegurate de tener remisiones rechazadas.", life: 6000,});
  } else {
    const { getDriverRejectedReferrals } = await useDriver();
    const stepByStep = await getDriverRejectedReferrals();
