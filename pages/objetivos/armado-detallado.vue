<template>
  <div class="w-full px-5 py-3">
    ...
    <form
      class="grid max-w-4xl grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3"
      @submit.prevent
    >
      ...
      <div class="flex flex-col gap-2 text-sm font-medium text-gray-700">
        <label for="detailed-assembly-file">Archivo Excel</label>
        <div class="flex flex-col gap-2 sm:flex-row">
          <input
            id="detailed-assembly-file"
            ref="fileInput"
            type="file"
            accept=".xlsx,.xls"
    ...
    <div v-if="previewRows.length" class="mt-6 max-w-4xl">
      <DinamicosTableFilters
        :headers="detailedAssemblyPreviewHeaders"
        :atributes="detailedAssemblyPreviewAttributes"
        :data="searchedPreviewRows"
        :loading="false"
        :warningData="detailedAssemblyPreviewWarningData"
        :errorData="false"
    ...
    <button
      ...
      @click="handleSave"
    >
      {{ isSaving ? "Guardando..." : "Guardar armado" }}
    </button>

    <Dialog
      v-model:visible="showReplaceDialog"
      modal
      :header="'Armado detallado existente'"
      :style="{ width: '32rem' }"
    >
      <div class="space-y-4">
        <p>
          Ya existe un armado detallado para la semana
        ...
        <div class="flex justify-end gap-2">
          ...
          >
            {{ isSaving ? "Actualizando..." : "Actualizar armado" }}
          </button>
        </div>
      </div>
    </Dialog>
...
import {
  ...
  type DetailedAssemblyPreviewData,
  type DetailedAssemblyPreviewRow,
  type DetailedAssemblySaveRequest,
  type DetailedAssemblyValidationErrors,
} from "~/composables/objetivos/useDetailedAssemblyApi";
import {
  detailedAssemblyPreviewAttributes,
  detailedAssemblyPreviewHeaders,
  detailedAssemblyPreviewWarningData,
...
const previewRows = ref<DetailedAssemblyPreviewRow[]>([]);
const previewContext = ref<Pick<
  DetailedAssemblyPreviewData,
  "year" | "week" | "username"
> | null>(null);
const validationErrors = ref<DetailedAssemblyValidationErrors>({});
const previewSearch = ref("");
const showReplaceDialog = ref(false);
const pendingSaveRequest = ref<DetailedAssemblySaveRequest | null>(null);
const username = useCookie<string | null>("usuario");
const { previewExcel, saveDetailedAssemblies } = useDetailedAssemblyApi();
...
const getSaveRequest = (): DetailedAssemblySaveRequest | null => {
...
const submitSaveRequest = async (saveRequest: DetailedAssemblySaveRequest) => {
  isSaving.value = true;

  try {
    const response = await saveDetailedAssemblies(saveRequest);
