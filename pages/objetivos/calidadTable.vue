<template>
  <div class="w-full p-3">
    <title>Consultar cumplimiento mensual</title>
    <RemisionesTabPanelRemisiones :items="itemsCumplimientoMensual" />
    <DinamicosTableFilters
      :headers="headersCumplimientoMensual"
      :atributes="attributesCumplimientoMensual"
      :data="data"
      :loading="isLoading"
...
import {
  attributesCumplimientoMensual,
  headersCumplimientoMensual,
  itemsCumplimientoMensual,
} from "../../composables/objetivos/datosObjetivos";
import { definePageMeta } from "../node_modules/nuxt/dist/pages/runtime/composables";

const toast = useToast();
const clientID = useCookie("idCliente");
const { listCalidades } = useObjetivosApi();
...
const fixData = (items: any[]) => items.map((item) => ({
  ...
  checklist: formatPercent(item.checklist),
  inspeccion: formatPercent(item.inspeccion),
}));
...
const list = async (startDate?: Date | null, endDate?: Date | null) => {
  ...
  const response = await listCalidades(
  ...
  if (response.success) {
    ...
    if (data.value.length === 0) {
      ...
      warningData.value.label = "Sin registros de cumplimiento mensual en el rango seleccionado.";
    }
  } else {
    errorData.value = true;
    toast.add({
      severity: "error",
      summary: "Error al cargar cumplimiento mensual.",
